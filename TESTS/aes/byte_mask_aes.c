...
uint8_t Sbox_masked[256] ALIGN;
uint8_t Mask[10] ALIGN;
uint8_t WordMask[4] ALIGN;
uint8_t RoundKey_masked[11][16] ALIGN;
uint8_t roundKey[176] ALIGN;
...
//#define ROR32(x,y) ((uint32_t)(x) >> (y) | (uint32_t)(x) << 32 - (y))

//uint32_t ror32(uint32_t x, uint32_t y)
//{
//    return ((uint32_t)(x) >> (y) | (uint32_t)(x) << 32 - (y)) ;
//}
...
void aes128(uint8_t* state)
{
    ...
    endtrigger();
    // use following to remove masks from state at this point (not part of AES)
    // remask(state,0,0,0,0,Mask[4]^WordMask[0],Mask[4]^WordMask[1],Mask[4]^WordMask[2],Mask[4]^WordMask[3]);
#ifdef TEST_AES
    }
    subBytes_masked(state);
    shiftRows(state);
